...
### BUG-01: В бою «не срабатывают» карты и не обновляется мана
Симптомы:
- перетаскивание карты на врага визуально происходит, но эффект не применяется
- энергия/мана не отображается или не меняется

Ожидаемая причина:
- сервер получает действие, но падает/не меняет state (см. BUG-03), либо фронт не получает корректный `combat_view`.

Критичность: **Blocker** (игра неиграбельна)

---

### BUG-02: Энциклопедия (кодекс) «разъезжается» и выходит за края экрана
Симптомы:
- модальное окно/контейнер шире вьюпорта
- сетка карточек не адаптируется

Ожидаемый фикс:
- ограничить высоту/ширину `overlay`/`panel` и добавить `overflow:auto`
- адаптивные колонки на больших/малых экранах

Критичность: **Major**

---

### BUG-03: После перезагрузки страницы теряется прогресс / невозможно продолжить, сейв бьётся
Симптомы:
- `json.decoder.JSONDecodeError` при `json.load`
- после ошибки сервер отвечает 500 на `/api/bootstrap` и `/api/action`
- новый забег не стартует, потому что сервер не может загрузить сейв

Ожидаемая причина:
- сейв‑файл был записан частично (неатомарная запись) или туда попал несериализуемый мусор

Ожидаемый фикс:
- атомарная запись сейва
- обработка битого сейва: если JSON не парсится — переименовать в `.corrupt` и создать новый
- добавить действие `CONTINUE` на фронте/сервере, чтобы реально возвращаться в нужный экран после refresh
...
- [ ] A3. Починить продолжение после refresh:
  - действие `CONTINUE` вызывается после bootstrap если есть активный run
  - корректное возвращение на COMBAT/REWARD/MAP и т.д.

### Этап B — UI/UX
- [ ] B1. Исправить кодекс (overflow + responsive grid)
- [ ] B2. Улучшить визуал дропа: подсветка цели, запрет розыгрыша при нехватке маны
- [ ] B3. Экран колоды: сортировка/фильтр по редкости/типу/тегам
...
- [ ] D1. Генерация графа путей (несколько линий, связи)
- [ ] D2. Визуализация линий/узлов на фронте
